...
async function getFutureEventByUserId(userId: string) {
  const { data, error } = await supabase.rpc("get_future_event", { input_user_id: userId });
...
Deno.serve(async (req) => {
  ...
  try {
    let eventorderDetails = await getFutureEventByUserId(userId??"");
    ...
    eventorderDetails= eventorderDetails.slice(1);
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/getfutureeventbyuserId' \
