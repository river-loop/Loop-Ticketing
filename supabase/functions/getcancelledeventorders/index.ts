...
async function getCancelledEventOrdersByUserId(userId: string) {
  const { data, error } = await supabase.rpc("get_cancelled_event_orders", { input_user_id: userId });
...
Deno.serve(async (req) => {
  ...
  try {
    const canceleventorderDetails = await getCancelledEventOrdersByUserId(userId??"");
    
    if (!canceleventorderDetails) {
      return new Response("Orders not found", { status: 404 });
    }

    return new Response(JSON.stringify({ orders: canceleventorderDetails }), {
      headers: {
        "Access-Control-Allow-Origin": Allow_origin_url_prd, // Allow only your specific domain
        "Access-Control-Allow-Methods": "POST, GET, OPTIONS", // Allowed methods
        "Access-Control-Allow-Headers": "Content-Type, Authorization",  // Allowed headers
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/getcancelledeventorders' \
